    ...
    <div class="col-md-12 mb-30">
        <div class="card card-statistics h-100">
            <div class="card-body">
                ...
                @endif
                <form method="post" action="{{ route('library.update', $library) }}" autocomplete="off"
                                ...
                                <select class="custom-select mr-sm-2" name="grade_id">
                                    ...
                                    @foreach ($grades as $grade)
                                        <option value="{{ $grade->id }}"
                                            {{ $grade->id == $library->grade_id ? 'selected' : '' }}>{{ $grade->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('grade_id')
                                        ...
                                        @if ($grade->id == $library->grade_id)
                                            @foreach ($grade->classrooms as $classroom)
                                                <option value="{{ $classroom->id }}"
                                                    {{ $classroom->id == $library->classroom_id ? 'selected' : '' }}>
